...
for f in freqlist:
    for name in freqsample[f]:
        ...
        if len(copy_new_data.dropna()) > len(copy_old_data.dropna()):
            if displayed <= display_num:
                ...
                result = pd.concat([result, pd.DataFrame([['updated:', True]])])
                result = pd.concat([result, blank])
                displayed += 1
            updated += 1
        if not old_data.dropna().equals(new_data.dropna()):
            ...
            modified_result = pd.concat([ modified_old_item, modified_new_item], ignore_index=True).T.dropna(axis=0, how="all")
...
print('\nRun Time: '+str(int(time.time() - tStart))+' s'+'\n')
print("Total Updated items = "+str(updated))
print("Total Modified items = "+str(modified))
result = pd.concat([result, blank])
result = pd.concat([result, pd.DataFrame([['Total Updated items:', updated]])])
